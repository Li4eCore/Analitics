...
**AVG OVER PARTITION по одному признаку.**
```sql
SELECT
    product_name,
    category_id,
...
> Важно: без явного `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`
> `LAST_VALUE` по умолчанию видит только строки до текущей включительно —
> результат будет неверным. Указывать полный фрейм здесь обязательно.
