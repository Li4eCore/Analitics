...
**Подзапрос с агрегатом внутри вычисления.**
```sql
SELECT
    product_id,
    product_name,
...
**Подзапрос через MAX** — товары из самого последнего заказа.
```sql
SELECT
    p.product_name
FROM products p
...
**Замена NULL в выводимом столбце.**
```sql
SELECT
    company_name,
...
**Защита от деления на ноль + приведение типа.**
```sql
SELECT
    last_name,
    total_score,
